using Renoza.CashReceiptBroker;
using Renoza.CashReceiptBroker.Configuration;
...
public class Program
{
    ...
    public static void Main(string[] args)
    {
        ...
        try
        {
            Log.Information("Запуск приложения Renoza.CashReceiptBroker");
            host.Build().Run();
        }
        catch (Exception ex)
        {
    ...
    private static IServiceCollection Register(HostBuilderContext context, IServiceCollection services)
    {
        ...
        return services
            .AddOptions()
            .AddHostedService<CashReceiptBroker>();
    }
}
